...
export async function startServer() {
  ...
  setTimeout(() => {
    setInterval(async () => {
      ...
      const watchers = await db.all<{ owner_uuid: string; term_id: string; crn_list: string }[]>(
        `SELECT owner_uuid, term_id, crn_list FROM watchers WHERE term_id IN (${mostRecentTerms?.map(() => "?").join(", ")})`,
        ...mostRecentTerms
      );
  ...
  setTimeout(
    () => {
      setInterval(async () => {
        ...
        await db.all("DELETE FROM watchers WHERE delete_warning_date IS NOT NULL AND (delete_warning_date + 604800) < ?", Math.floor(Date.now() / 1000));

        const outdatedWatchers = await db.all<{ owner_uuid: string }[]>(
          `UPDATE watchers SET delete_warning_date = ? WHERE term_id NOT IN (${mostRecentTerms?.map(() => "?").join(", ")}) AND delete_warning_date IS NULL RETURNING owner_uuid`,
