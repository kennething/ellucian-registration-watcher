...
router.delete("/watcher/delete", authController, async (req, res) => {
  ...
  const parsedSchedules = schedules.flatMap((schedule) => ({ ...schedule, crns: JSON.parse(schedule.crns) as string[] }));
  if (parsedSchedules.some((schedule) => schedule.term_id === fetchedWatcher.term_id && schedule.crns.includes(fetchedWatcher.crn)))
    return res.status(400).json({ error: "Cannot delete watcher for a class that is in a schedule" });
